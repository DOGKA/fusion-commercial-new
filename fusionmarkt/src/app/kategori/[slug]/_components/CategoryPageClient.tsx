...
export interface CategoryInitialData {
  ...
  isBundle: boolean;
  /** Sunucunun kullandığı sıralama; istemci bunu değiştirene kadar fetch yok. */
  sort: SortOption;
}

/**
 * Kategori sayfasının tamamı.
 *
 * Veri artık sunucudan `initialData` ile geliyor (bkz. `page.tsx`). Eskiden
 * liste `useEffect` içinde çekiliyordu; o hâlde sunucunun ürettiği HTML boştu
 * ve arama motorları ile LLM tarayıcıları kategoride hiçbir ürün göremiyordu.
 * Prop olarak gelince ilk HTML dolu geliyor, hidrasyon sonrası davranış aynı.
 */
export default function CategoryPageClient({
  ...
  const [isBundleCategory, setIsBundleCategory] = useState(initialData.isBundle);
  ...
  const [sortBy, setSortBy] = useState<SortOption>(initialData.sort);
  ...
  /**
   * Sunucu listeyi zaten gönderdi; mount'taki isteği atla. Aynı sorguyu
   * hidrasyondan hemen sonra tekrarlamak dolu ızgarayı bir kare iskelete
   * çevirirdi. Sıralama değişince (veya başka kategoriye geçilince) effect
   * normal şekilde çalışıyor.
   */
  const skipInitialFetch = useRef(
    initialData.products.length > 0 && initialData.sort === sortBy
  );
  ...
  /**
   * Sıralama/sayfa URL'den okunuyor ama `useSearchParams` ile değil.
   *
   * Sayfa statik üretiliyor: o kancayı kullanmak ızgarayı en yakın Suspense
   * sınırına kadar istemci render'ına düşürür ve sunucudan gelen HTML yeniden
   * ürünsüz kalırdı. Paylaşılan bağlantılarda sıralama nadir olduğu için
   * varsayılan liste basılıyor, farklı bir sıralama varsa mount'tan sonra
   * uygulanıyor ve yukarıdaki effect listeyi tazeliyor.
   */
  useEffect(() => {
    const query = new URLSearchParams(window.location.search);
    const urlSort = getValidSortOption(query.get("sort"));
    const urlPage = Number.parseInt(query.get("page") || "1", 10);
    if (urlSort !== sortBy) setSortBy(urlSort);
    if (Number.isFinite(urlPage) && urlPage > 1) setCurrentPage(urlPage);
    // Yalnızca ilk yüklemede: sonraki değişiklikler updateURL üzerinden yürüyor.
