...
export default function Header() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/magaza?q=${encodeURIComponent(searchQuery.trim())}`);
    ...
    <>
      <header
        className={cn(
          ...
          isScrolled 
            ? "bg-black/40 backdrop-blur-2xl border-b border-white/[0.05] py-2" 
            : "bg-transparent py-4"
        )}
      >
        {/* Subtle gradient line at top */}
        <div className={cn(
          "absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/10 to-transparent transition-opacity duration-500",
          ...
          <div className="flex items-center justify-between" style={{ gap: '30px' }}>
                  ...
                  >
                    <button
                      className={cn(
                        ...
                        "before:absolute before:inset-0 before:rounded-lg before:bg-white/0 before:transition-all before:duration-300",
                        "hover:before:bg-white/[0.05]",
                        activeDropdown === item.name && "text-white before:bg-white/[0.05]"
                      )}
                    >
                    ...
                    {activeDropdown === item.name && (
                      <div className="absolute top-full left-0 pt-2">
                        <div className="bg-black/80 backdrop-blur-xl border border-white/[0.08] rounded-xl py-2 min-w-[220px] shadow-2xl">
                          {item.submenu.map((sub) => (
                            <Link
                              key={sub.name}
                              href={sub.href}
                              onClick={() => setActiveDropdown(null)}
                              className="block px-4 py-2.5 text-[13px] text-white/60 hover:text-white hover:bg-white/[0.05] transition-all duration-200"
                            >
                              {sub.name}
                            </Link>
                          ))}
            ...
            <div className="flex items-center gap-0.5">
              ...
              <button
                ...
                className={cn(
                  "hidden lg:flex relative p-2 rounded-xl text-white/60 hover:text-white transition-all duration-300",
              ...
              <Link
                href="/hesabim"
                className={cn(
                  "relative flex items-center justify-center p-2.5 rounded-xl text-white/60 hover:text-white transition-all duration-300",
                  "before:absolute before:inset-0 before:rounded-xl before:bg-white/0 before:transition-all before:duration-300",
                  "hover:before:bg-white/[0.05]"
                )}
                aria-label="Hesabım"
        ...
        <div className="fixed inset-0 z-[60] flex items-start justify-center pt-24">
          ...
          <div className="relative w-full max-w-2xl mx-4 animate-in fade-in slide-in-from-top-4 duration-200">
            <form onSubmit={handleSearch} className="relative">
              <div className="relative bg-white/10 backdrop-blur-2xl border border-white/20 rounded-2xl overflow-hidden shadow-2xl">
                ...
                <input
                  ...
                  className="w-full bg-transparent text-white placeholder:text-white/40 py-5 pl-14 pr-24 text-lg outline-none"
                />
                <button
                  type="submit"
                  className="absolute right-3 top-1/2 -translate-y-1/2 px-4 py-2 bg-[var(--fusion-primary)] hover:bg-[var(--fusion-primary-hover)] text-white text-sm font-medium rounded-xl transition-colors"
                >
                  Ara
                </button>
              </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2 justify-center">
              <span className="text-white/40 text-sm">Popüler:</span>
              {["Güç Kaynağı", "Güneş Paneli", "Eldiven", "Merdiven"].map((term) => (
                <button
                  key={term}
                  onClick={() => {
                    setSearchQuery(term);
                    router.push(`/magaza?q=${encodeURIComponent(term)}`);
                    setIsSearchOpen(false);
                  }}
            ...
            <p className="mt-6 text-center text-white/30 text-sm">
              Kapatmak için <kbd className="px-2 py-0.5 bg-white/10 rounded text-white/50">ESC</kbd> tuşuna basın
            </p>
          </div>
        </div>
      )}
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-x-0 top-[56px] z-40 lg:hidden mobile-dropdown-menu">
          <div
            className="fixed inset-0 top-[56px] bg-black/70 backdrop-blur-md"
            onClick={() => setIsMobileMenuOpen(false)}
          />

          <div className="relative mx-3 mt-3 overflow-hidden rounded-3xl shadow-2xl shadow-black/50">
            <div className="absolute inset-0 bg-gradient-to-b from-white/[0.08] to-white/[0.02] backdrop-blur-2xl" />
            <div className="absolute inset-0 bg-[#0a0a0a]/80" />
            <div className="absolute inset-0 rounded-3xl border border-white/[0.12]" />
            ...
            <nav className="relative p-4 space-y-1">
              ...
              <form onSubmit={handleSearch} className="mb-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white/[0.05] border border-white/[0.08] text-white placeholder:text-white/40 py-3 pl-11 pr-4 rounded-xl text-sm outline-none focus:border-white/20"
                  />
                </div>
              </form>
                ...
                item.submenu ? (
                  <div key={item.name}>
                    ...
                    >
                      {item.name}
                      <ChevronDown className={cn(
                        "w-4 h-4 transition-transform duration-300 text-white/40",
                      ...
                      <div className="mt-1 ml-4 space-y-0.5 pb-2">
                        {item.submenu.map((sub) => (
                          <Link
                            ...
                            href={sub.href}
                            onClick={() => setIsMobileMenuOpen(false)}
                            className="flex items-center gap-2 px-4 py-2.5 text-[14px] text-white/50 hover:text-white rounded-lg transition-colors"
                          >
              ...
              <div className="grid grid-cols-2 gap-2">
                <Link
                  href="/favori"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="flex items-center justify-center gap-2 px-4 py-3 text-[13px] font-medium text-white/60 hover:text-white bg-white/[0.03] hover:bg-white/[0.06] rounded-xl border border-white/[0.05] transition-all"
                >
