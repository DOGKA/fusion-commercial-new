...
export default function SingleProductView({ slug, initialData }: SingleProductViewProps) {
  ...
  // Seçenek seçilmeden sepete/favoriye eklenmek istendiğinde seçenekleri vurgular
  ...
  // Sibling products (prev/next in same category)
  ...
  const warnMissingVariant = useCallback(() => {
    setVariantWarning(true);
    setTimeout(() => setVariantWarning(false), 2500);
  }, []);
  ...
  // Fetch sibling products (prev/next in same category)
  useEffect(() => {
    if (!slug || !productData?.category) return;
    const fetchSiblings = async () => {
      try {
        const res = await fetch(`/api/public/products/${slug}/siblings`);
        if (res.ok) {
          const data = await res.json();
  ...
  const [canReview, setCanReview] = useState(false); // Yorum yapma yetkisi var mı (üye + ürünü satın almış)
  ...
  // Yorum yapma uygunluğunu kontrol et (üye + ürünü satın almış olma şartı)
  useEffect(() => {
    const pid = productData?.id;
    if (!pid) return;

    fetch(`/api/reviews/eligibility?productId=${pid}`)
      .then(res => res.json())
      .then(data => {
  ...
  // Yorum gönderme (sadece üye + ürünü satın almış kullanıcılar)
          ...
          <div className="product-info-column" style={{
            ...
            {product.variants && product.variants.length > 0 && (
              <div style={{ marginBottom: '0px' }}>
                <span style={{ fontSize: '10px', color: variantWarning ? '#EF4444' : 'var(--foreground-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '10px', display: 'block' }}>
                  {variantWarning ? 'Lütfen bir seçenek seçin' : 'Seçenekler'}
                </span>
                  ...
                  <>
                    ...
                    {/* CTA Buttons */}
                        ...
                        {(() => {
                          ...
                          return (
                            <AddToCartButton
                              ...
                              onNeedsVariant={warnMissingVariant}
                            />
                          );
                        })()}
                      </div>
                      ...
                      {(() => {
                        ...
                        const productHasVariants = Boolean(product.variants?.length);
                        ...
                        return (
                          <button
                            type="button"
                            onClick={() => {
                              // Varyantlı üründe seçeneksiz favori, favoriden
                              // sepete eklenemeyen bir kalem üretirdi.
                              ...
                              toggleItem({
                                ...
                                requiresVariant: productHasVariants,
                              });
                            }}
                            title={
                    ...
                    reviewSubmitted ? (
                      ...
                      <div style={{
                        ...
                        <p style={{ fontSize: '13px', color: 'var(--foreground-secondary)', margin: 0, textAlign: 'center' }}>
                          Yorum yapabilmek için giriş yapmalısınız. Yalnızca bu ürünü satın alan üyelerimiz yorum yapabilir.
                        </p>
                        <a
                          href="/hesabim"
                          style={{
                      ...
                      // Giriş yapmış ama ürünü satın almamış kullanıcı
                      <div style={{
                        width: '100%',
                        padding: '14px 24px',
                        display: 'flex',
