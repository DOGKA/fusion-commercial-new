        ...
        <div className="relative w-full h-full bg-gray-900">
          ...
          <div className={`relative h-full flex flex-col justify-center p-6 ${alignClass}`}>
            ...
            <h2 className={`text-white font-bold mb-2 ${isMobile ? "text-lg" : "text-xl"}`}>
              ...
              {formData.titleHighlight && <span className="bg-gradient-to-r from-fm-green to-fm-cyan bg-clip-text text-transparent"> {formData.titleHighlight}</span>}
            </h2>
            {formData.subtitle && <p className={`text-white/70 mb-4 ${isMobile ? "text-xs" : "text-sm"}`}>{formData.subtitle}</p>}
            <div className="flex gap-2">
              {formData.buttonText && <span className={`bg-white text-dark rounded-full font-medium ${isMobile ? "text-[10px] px-3 py-1" : "text-xs px-4 py-1.5"}`}>{formData.buttonText}</span>}
              {formData.button2Text && <span className={`border border-white/50 text-white rounded-full font-medium ${isMobile ? "text-[10px] px-3 py-1" : "text-xs px-4 py-1.5"}`}>{formData.button2Text}</span>}
            </div>
          </div>
