...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await prisma.homepagePromo.findMany({
      orderBy: { order: "asc" },
    });

    return NextResponse.json({ items });
  } catch (error) {
    console.error("❌ [HOMEPAGE PROMOS API] List error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.homepagePromo.create({ data: { ...body } });

    revalidateTag("homepage");

    return NextResponse.json(item, { status: 201 });
  } catch (error) {
    console.error("❌ [HOMEPAGE PROMOS API] Create error:", error);
